let managerRole = {
    run: function () {
        ...
        if (harvesters.length / 2 > builder.length) {
            ...
            Game.spawns['Spawn'].spawnCreep([WORK, MOVE, CARRY, CARRY, CARRY], newName,
                {memory: {role: 'builder'}});
        }
        else if (upgrader.length < builder.length){
            let newName = 'Upgrader' + Game.time;
            console.log('Spawning new Upgrader: ' + newName);
            Game.spawns['Spawn'].spawnCreep([WORK, CARRY, CARRY, MOVE, CARRY], newName,
                {memory: {role: 'upgrader'}});
        }
        else{
            let newName = 'Harversters' + Game.time;
            console.log('Spawning new Harverster' + newName);
            Game.spawns['Spawn'].spawnCreep( [CARRY, CARRY, MOVE, WORK, CARRY], newName,
